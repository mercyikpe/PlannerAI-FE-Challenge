...
export function filterByStoreProductDate<T extends Order|Recommendation|Delivery>(list: T[], filter: StoreProductDateFilter): T[] {
    return list.filter((item) => {
        ...
        return selected
            && dayjs(item.target_date).isBetween(dayjs(currentDate), dayjs(nextWeek));
    });
}
