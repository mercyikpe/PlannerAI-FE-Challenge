<template>
    <header class="my-5">
        <h1 class="text-3xl font-bold text-center">
            AI Recommendations, Deliverables vs Actual Demand
        </h1>
    </header>

    <main class="mt-20 w-full mx-auto max-w-[2000px]">
        <section class="max-w-5xl mx-auto flex justify-between">
            ...
            <div class="w-full">
                <div class="space-y-0.5 w-1/2 ml-auto">
                    ...
                    >
                        ...
                        <option v-for="dateOption in dateRangeOptions" :key="dateOption">{{ dateOption }}</option>
                    </select>
...
import {createDateRange, filterByStoreProductDate, sortDatasetsByTargetDate, StoreProductDateFilter} from "@/util";
...
const dateRangeOptions = computed(() => {
    ...
    while (currentDate.isBefore(dayjs(allDates[allDates.length - 1]))) {
        ...
        dateOptions.push(
            createDateRange(currentDate.format('YYYY-MM-DD'), nextWeek.subtract(1, 'day').format('YYYY-MM-DD'))
        );
...
const datasets = computed((): ChartDataset[] => {
    ...
    for (const recommendation of recommendations.value) {
        ...
        if (matchingSales) {
            ...
            improved = matchingDelivery.delivery_qty - actualDemand < recommendation.recommendation - actualDemand;
        }
